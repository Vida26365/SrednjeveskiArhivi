...
fn InputPersons(document: DocumentParam, persons: PersonsParam) -> Element {
    ...
    rsx! {
        ...
        for (oseba, alliases) in persons.read().iter().cloned() {
            div {
                class: "flex gap-2",
                div {
                ...
                div {
                    for vzdevek in alliases.iter().cloned() {
                        div {
            ...
            // for alias in alliases.read()[index].iter().cloned() {
        ...
        div {
            ...
            div {
                button {
                    class: "cursor-pointer text-base-content/50 hover:text-base-content",
                    onclick: move |event| println!("Gumb {event:?}"), "+"
                }
            }
        }
    }
